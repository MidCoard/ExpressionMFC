...
public class SimpleDerivative extends Derivative {
    ...
    public @NonNull Simplifiable simplify() {
        if (this.getOriginalExpression() instanceof SimpleConstable)
            ...
            return new Fraction(Operator.MINUS.operate(Operator.MULTIPLY.operate(new Derivative(((SimpleFraction) this.getOriginalExpression()).getNumerator(),this.getArgument()),((SimpleFraction) this.getOriginalExpression()).getDenominator()),Operator.MULTIPLY.operate(((SimpleFraction) this.getOriginalExpression()).getNumerator(),new Derivative(((SimpleFraction) this.getOriginalExpression()).getDenominator(),this.getArgument()))), Operator.MULTIPLY.operate(((SimpleFraction) this.getOriginalExpression()).getDenominator(),((SimpleFraction) this.getOriginalExpression()).getDenominator())).simplify();
